...
4. **visual 轨素材选取**（演播室模式会自动填充空白时段，不需要强制全程覆盖）：
    ...
    - 有 _video_path + _video_segments 时，根据转录内容**语义匹配**选取 `time_range`：
...
- visual 轨必须全程覆盖（无空白）
- **visual 素材归属规则（极重要）**：每条新闻时段内的 visual 只能使用该新闻自己的素材（图片/视频），严禁用其他新闻的素材来填充空白！如果当前新闻的素材不够填满空白，用该新闻的图片重复显示或用 remotion 组件（highlight_text/info_panel 等）填充，绝对不要跨新闻"借"素材
        ...
        for i, item in enumerate(topics_with_data):
            ...
            if sd:
                ...
                if video_path:
                    ...
                    # 传递句子级别带时间戳的 transcript，供 LLM 精确选取 time_range
                    video_segments = sd.get("_video_segments", [])
                    if video_segments:
                        segments_text += f"- 视频逐句转录({len(video_segments)}句，time_range 必须从这里选！):\n"
                        ...
                        # 没有结构化转录时，用纯文本 transcript 作为参考
                        video_transcript_text = sd.get("_video_transcript", "")
                        if video_transcript_text:
                            segments_text += f"- 视频转录文本(无精确时间戳，仅供参考): {video_transcript_text[:500]}\n"
...
### 视频素材使用规则：
- **time_range 必须基于 _video_segments 时间戳**，按语义选取完整的句子片段
- time_range = [某句的start, 某句的end]，不要自己编造时间，不要固定10秒
- 可合并多个相邻句子: time_range = [第1句.start, 第3句.end]
- duration_ms 必须等于 (time_range[1] - time_range[0]) * 1000
- play_audio=true 的原声片段：从 _video_segments 中选取**最精华**的几句话，不要全时长播放
...
- **禁止**：time_range 从 0 开始、固定 10 秒、不看 segments 内容就瞎选、截取半句话
    ...
    def _load_source_data(self, topic: dict, collected_dir: Path, manifest: dict = None) -> dict | None:
        ...
        if manifest and source_file in manifest:
            ...
            if video_info:
                ...
                # 每条是一句完整的话，适合 LLM 按语义选取 time_range
                transcript = video_info.get("transcript", "")
                if isinstance(transcript, list) and transcript:
                    source_data["_video_segments"] = transcript
                    # 同时提供纯文本版本方便 LLM 理解全文
                    source_data["_video_transcript"] = video_info.get("transcript_text", "")
                else:
                    # 没有结构化 transcript 时，用 transcript_hint（recognizer 估计的文本）
