...
## 角色设定
- 角色：毒舌四川话 AI VTuber "Mili"（二次元角色，Live2D 呈现）
- 风格：尖酸刻薄但有趣，用四川方言吐槽，偶尔夹杂普通话
- 目标：让观众觉得好笑、有共鸣，愿意评论互动
...
class DirectorAgent:
    ...
    def select_topics(self, collected_dir: Path) -> dict:
        """
        ...
        if hot_platforms:
            hot_summary = self._build_summary(hot_platforms)
        ...
        if ai_platforms:
            ai_summary = self._build_summary(ai_platforms)
            # 附加 rankings 数据
...
- visual 轨和 voice 轨要有时间重叠（说话同时展示素材）"""
    ...
    def _aggregated_system_prompt(self, video_type: str = "hot_daily") -> str:
        ...
        if video_type == "ai_daily":
            persona = """## 关于 Mili（AI日报模式）
- 二次元 Live2D 角色，科技达人四川妹子
- 说话风格：**专业但不枯燥**，像一个懂技术的朋友在给你安利好工具
- 语言：四川话为主，技术术语保留英文原文
- 重点要求：
  - 每个项目要讲清楚**是什么、能干什么、适合谁用**
  - 多说技术细节：模型参数量、训练方法、支持语言、部署方式等
  - 给出使用场景举例（\"比如你要做XXX，就可以用这个\"）
  - 对比同类工具的差异（如果知道的话）
  - 数据要具体：star 数、下载量、更新频率等
  - 可以用类比帮助理解（\"简单说就是XXX的开源版\"）"""
        else:
            persona = """## 关于 Mili（热搜模式）
- 二次元 Live2D 角色，八卦达人四川妹子
- 说话风格：**搞笑、接地气、正能量**，像茶馆里那个最会摆龙门阵的妹子
- 语言：地道四川话为主，多用俏皮话和网络热梗
- 重点要求：
  - 吐槽要有趣但不恶毒，笑点要自然
  - 正能量收尾：即使是负面新闻也要找到积极角度或幽默化解
  - 多用生活化类比让热搜话题更接地气
  - 适当加入互动引导（\"你们觉得呢？\"\"评论区说说看\"）
  - 热搜评论要挑有趣的展示
  - 节奏要快，不要拖沓，像朋友给你分享今天最离谱的事"""
...
- 台词必须通过抖音违禁词检查"""
    ...
    def _build_summary(self, platforms: dict) -> str:
        """为选题 LLM 构建素材摘要"""
        parts = []
        for platform, items in platforms.items():
            ...
            for i, item in enumerate(items):
                ...
                parts.append(f"  {i+1}. [{source_file}] {title} (热度:{hot}) {media_str}")
                if content:
